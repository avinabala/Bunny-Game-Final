...
function setup() {
  ...
  youWin = createSprite(500, 250, 10, 10)
...
function draw() {
  background(bgImg);
    ...
    if(CarrotsCollected === 20){
      foxGroup.destroyEach();
      carrotGroup.destroyEach();
      fill("#1C1539")
      text("CarrotsCollected: "+ CarrotsCollected, -100,-100);
      youWin.visible = true
      gameState = END
      gameOver.visible = false
    }
...
function spawnFox(){
  if (frameCount % Math.round(random(200,800)) === 0){
    ...
    var fox = createSprite(Math.round(random(200,800)),355,10,40);
